 ...
 *
 * DHT_GPIO_PIN and LED_GPIO_PIN are AP GPIOA line numbers.  The board-side
 * preparation script must place both pads in GPIO mode before this firmware
 * starts, and the two pins must remain electrically separate.
 */
...
#if DHT_GPIO_PIN > 31 || LED_GPIO_PIN > 31 || DHT_GPIO_PIN == LED_GPIO_PIN
#error "DHT_GPIO_PIN and LED_GPIO_PIN must be distinct GPIOA lines in 0..31"
#endif
...
{
    uint32_t value = robot_read(GPIOA_BASE + GPIO_DATA);
    uint32_t mask = (uint32_t) 1U << pin;

    robot_write(GPIOA_BASE + GPIO_DATA, high ? value | mask : value & ~mask);
}

static void gpio_output(uint8_t pin)
{
    uint32_t value = robot_read(GPIOA_BASE + GPIO_DIRECTION);

    robot_write(GPIOA_BASE + GPIO_DIRECTION, value | ((uint32_t) 1U << pin));
}
...
{
    return (uint8_t) ((robot_read(GPIOA_BASE + GPIO_EXTERNAL_PORT) >> pin) & 1U);
}
...
static uint8_t wait_for_level(uint8_t level)
{
    ...
    while (polls-- != 0U) {
        if (gpio_read(DHT_GPIO_PIN) == level) {
            return 1;
        }
    }
    return 0;
...
static uint8_t dht11_read(uint8_t bytes[5])
{
    ...
    gpio_write(DHT_GPIO_PIN, 0);
    gpio_output(DHT_GPIO_PIN);
    delay_ms(DHT_START_LOW_MS);
    gpio_input(DHT_GPIO_PIN);
    ...
    for (byte_index = 0; byte_index < 5; ++byte_index) {
        ...
        for (bit_index = 0; bit_index < 8; ++bit_index) {
            ...
            value = (uint8_t) ((value << 1) | gpio_read(DHT_GPIO_PIN));
            if (!wait_for_level(0)) {
                return 2;
            }
        }
